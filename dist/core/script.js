 ...
 */
export function resolveScript(name, config, tabConfig) {
 ...
 */
export async function executeScript(scriptDef, args, options = {}) {
    ...
    const ext = scriptDef.runtime === "python3"
        ? ".py"
        : scriptDef.runtime === "node"
            ? ".mjs"
            : ".sh";
    const tmpFile = path.join(tmpDir, `rowbound_script_${Date.now()}_${Math.random().toString(36).slice(2)}${ext}`);
    const scriptContent = `${shebang}\n${scriptDef.code}\n`;
    fs.writeFileSync(tmpFile, scriptContent, { mode: 0o755 });
    try {
        return await new Promise((resolve, reject) => {
            const child = execFile(tmpFile, args, {
                timeout,
                signal,
                env: env
                    ? { ...process.env, ...env }
 ...
 */
export async function executeScriptAction(scriptDef, args, options = {}) {
