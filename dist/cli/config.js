...
export function registerConfig(program) {
    ...
    configCmd
        ...
        try {
            let scriptDef;
            try {
                scriptDef = JSON.parse(opts.json);
            }
            catch {
            ...
            if (!scriptDef.runtime || !scriptDef.code) {
                console.error(error("Script must have at least 'runtime' and 'code' fields."));
                process.exitCode = 1;
                return;
            }
            const existing = await adapter.readConfig(ref);
            ...
            if (existing.tabs) {
                ...
                if (tab.scripts[opts.name]) {
                    console.error(error(`Script with name "${opts.name}" already exists.`));
                    process.exitCode = 1;
                    return;
                }
