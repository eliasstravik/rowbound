...
import { readScheduleState, scheduleKey, updateScheduleEntry, } from "../core/source-schedule.js";
...
export function registerSource(program) {
    ...
    source
        ...
        try {
            ...
            if (!config) {
                logErr(`${error("No Rowbound config found.")} Run 'rowbound init <sheetId>' first.`);
                process.exitCode = 1;
                return;
            }
            ...
            if (sources.length === 0) {
                logErr(`${error("No sources configured.")} Add sources with 'rowbound config add-source'.`);
                process.exitCode = 1;
                return;
            }
            // Find source to run
            ...
            if (opts.source) {
                ...
                if (!found) {
                    logErr(`${error(`Source "${opts.source}" not found.`)} Available: ${sources.map((s) => s.id).join(", ")}`);
                    process.exitCode = 1;
                    return;
                }
                sourcesToRun = [found];
            ...
            const rateLimiter = reconciled.config.settings.rateLimit > 0
            ...
            for (const src of sourcesToRun) {
                ...
                const result = await executeSource(src, {
                    ...
                    env,
                    dryRun: opts.dryRun,
                    resolveScript: (name) => resolveScript(name, reconciled.config, tabConfig),
                    rateLimiter,
                ...
                updateScheduleEntry(sheetId, tabGid, src.id, result.errors.length > 0 ? "failed" : "completed", result.rowsCreated);
                if (jsonMode) {
                    console.log(JSON.stringify(result, null, 2));
                }
                else {
                    log(`  ${success("✓")} Created: ${bold(String(result.rowsCreated))}` +
    ...
    source
        ...
        try {
            ...
            for (const src of sources) {
                const schedule = src.type !== "webhook"
                    ? (src.schedule ?? "manual")
                    : "webhook";
                ...
                const lastRun = entry
                    ...
                    : "never run";
                console.log(`  ${bold(src.id)} (${src.type}) — ${schedule} — ${dim(lastRun)}`);
                console.log(`    columns: ${Object.keys(src.columns).join(", ")}`);
                if (src.dedup) {
                    console.log(`    dedup: ${src.dedup}${src.updateExisting ? " (update existing)" : ""}`);
                }
            }
        }
        catch (err) {
            console.error(error("Failed:"), err instanceof Error ? err.message : String(err));
            process.exitCode = 1;
        }
    });
}
