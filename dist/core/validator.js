...
export function validateConfig(config) {
    ...
    for (const source of sources) {
        ...
        if (!source.type || !VALID_SOURCE_TYPES.has(source.type)) {
            errors.push(`${label}: invalid type "${source.type}" (expected: http, exec, webhook)`);
        }
        if (!source.columns ||
        ...
        if (source.type === "http") {
            ...
            if (httpSource.extract && !isValidJsonPath(httpSource.extract)) {
                errors.push(`${label}: invalid JSONPath in 'extract': "${httpSource.extract}"`);
            }
            if (httpSource.extractPath && !isValidJsonPath(httpSource.extractPath)) {
                errors.push(`${label}: invalid JSONPath in 'extractPath': "${httpSource.extractPath}"`);
            }
            ...
            if (execSource.extract && !isValidJsonPath(execSource.extract)) {
                errors.push(`${label}: invalid JSONPath in 'extract': "${execSource.extract}"`);
            }
        ...
        if (source.type !== "webhook") {
            ...
            if (s.schedule &&
                ...
                warnings.push(`${label}: schedule "${s.schedule}" is not a known schedule (known: manual, hourly, daily, weekly, or cron expression)`);
            }
        }
    }
